...
var euiEcharts = (function(){
	...
	var _getOption = function(_customOption,chartType) {//获取option对象
		...
		(function($){
			...
			/*var xAxis = _extend(option.xAxis,__option.xAxis);
			if (_customOption.xAxis instanceof Array) {
				xAxis = _extend(xAxis,_customOption.xAxis);
			}else{
				xAxis = _extend(xAxis[0],_customOption.xAxis);
			}*/
	...
	var _getOptionByType = function(_type) {//获取option对象
		...
		if(_type == "line" || _type == "bar"){
			...
			(function($){
				 ...
				 * 为option增加默认设置，全局的默认设置
				 */
				//var xAxis = _extend(_option.xAxis,__option.xAxis);
	...
	var resize = function(chartId){//手动触发resize方法
		var _chart = _CacheFun._getCache(chartId);
		if (_chart != null && _chart) {
			_chart.resize();
		}
