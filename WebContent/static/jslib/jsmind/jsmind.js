/*
 * Released under BSD License Copyright (c) 2014 hizzgdev@163.com
 * 
 * Project Home: https://github.com/hizzgdev/jsmind/
 */
...
(function($w) {
	...
	var _console = {}, applyConsole = function(names) {
		for (var i = 0; i < names.length; i++) {
			var _n = names[i];
			_console[_n] = (function(n) {
				return function() {
					(n != 'debug' || globalDebug) && console && console[n]
							&& console[n](arguments[0]);
				}
			})(_n)
		}
	};
	applyConsole(['info', 'log', 'warn', 'error', 'debug']);
	...
	var DEFAULT_OPTIONS = {
		...
		// 扩展面板
		// 默认不添加扩展面板
		// position:位置,可选项left,right
		// width:宽度,可选项固定宽度或者百分比
		// 例子:
		// extensionPanel:{
		// position:'left',
		// width:'200px'
	...
	var jm = function(options) {
		...
		if (!THEME_OPTIONS[opts.theme]) {
			opts.theme = 'default';
		}
		globalDebug = opts.debug === true;
	...
	jm.node = function(sId, iIndex, sTopic, oData, bIsRoot, oParent,
			eDirection, level, leaf) {
		if (!sId) {
			_console.error('invalid nodeid');
			return;
		}
		if (typeof(iIndex) != 'number') {
			_console.error('invalid node index');
			return;
		}
		if (bIsRoot) {
			level = 0;
	...
	jm.node.prototype = {
		...
		getPath : function() {
			var me = this;
			if (me.parent) {
				return me.parent.getPath() + '-' + me.id
			} else {
				return me.id
			}
		}
	...
	jm.mind.prototype = {
		...
		add_node : function(parent_node, nodeid, topic, data, idx, direction,
			...
			if (typeof parent_node == 'string') {
				return this.add_node(this.get_node(parent_node), nodeid, topic,
						data, idx, direction, leaf);
			}
			var nodeindex = idx || -1;
			if (!!parent_node) {
				...
				if (parent_node.isroot) {
					...
					if (isNaN(direction)) {
						...
						d = (children_len > 1 && r > 0)
								? jm.direction.left
								: jm.direction.right
					} else {
						d = (direction != jm.direction.left)
								? jm.direction.right
								: jm.direction.left;
					}
					node = new jm.node(nodeid, nodeindex, topic, data, false,
							parent_node, d, parent_node.level + 1, leaf);
				} else {
					node = new jm.node(nodeid, nodeindex, topic, data, false,
							parent_node, parent_node.direction,
							parent_node.level + 1, leaf);
				}
				if (this._put_node(node)) {
					...
					_console.error('fail, the nodeid \'' + node.id
							+ '\' has been already exist.');
					node = null;
				}
				return node;
			} else {
		...
		insert_node_before : function(node_before, nodeid, topic, data, leaf) {
			if (typeof node_before == 'string') {
				return this.insert_node_before(this.get_node(node_before),
						nodeid, topic, data, leaf);
			}
			if (!!node_before) {
				var node_index = node_before.index - 0.5;
				return this.add_node(node_before.parent, nodeid, topic, data,
						node_index, leaf);
			} else {
		...
		insert_node_after : function(node_after, nodeid, topic, data, leaf) {
			if (typeof node_after == 'string') {
				return this.insert_node_after(this.get_node(node_after),
						nodeid, topic, data, leaf);
			}
			if (!!node_after) {
				var node_index = node_after.index + 0.5;
				return this.add_node(node_after.parent, nodeid, topic, data,
						node_index, leaf);
			} else {
		...
		move_node : function(node, beforeid) {
			...
			if (!!node && !!beforeid) {
				if (beforeid == '_last_') {
					...
					var node_before = (!!beforeid)
							? this.get_node(beforeid)
							: null;
					if (node_before != null && node_before.parent != null
		...
		_put_node : function(node) {
			if (node.id in this.nodes) {
				_console.warn('the nodeid \'' + node.id
						+ '\' has been already exist.');
				return false;
			} else {
	...
	jm.format = {
		node_tree : {
			...
			_extract_subnode : function(mind, node_parent, node_json) {
				...
				if (node_parent.isroot) {
					d = node_json.direction == 'left'
							? jm.direction.left
							: jm.direction.right;
				}
				if (node_json.leaf == undefined) {
					node_json.leaf = !(node_json.children && node_json.children.length);
				}
				var node = mind.add_node(node_parent, node_json.id,
						node_json.topic, data, null, d, node_json.leaf);
			...
			_buildnode : function(node) {
				...
				if (!!node.parent && node.parent.isroot) {
					o.direction = node.direction == jm.direction.left
							? 'left'
							: 'right';
				}
				if (node.data != null) {
		...
		node_array : {
			...
			_extract_subnode : function(mind, parentid, node_array) {
				...
				while (i--) {
					...
					if (node_json.parentid == parentid) {
						...
						mind.add_node(parentid, node_json.id, node_json.topic,
								data, null, d, node_json.leaf);
						node_array.splice(i, 1);
						extract_count++;
						var sub_extract_count = df._extract_subnode(mind,
								node_json.id, node_array);
			...
			_extract_data : function(node_json) {
				var data = {};
				for (var k in node_json) {
					if (k == 'id' || k == 'topic' || k == 'parentid'
							|| k == 'isroot' || k == 'direction') {
						continue;
					}
					data[k] = node_json[k];
				}
		...
		freemind : {
			...
			_load_node : function(mind, parent_id, xml_node) {
				...
				if (!!node_position) {
					node_direction = node_position == 'left'
							? jm.direction.left
							: jm.direction.right;
				}
				// _console.debug(node_position +':'+ node_direction);
				if (!!parent_id) {
					mind.add_node(parent_id, node_id, node_topic, node_data,
							null, node_direction, !xml_node.childNodes.length);
				} else {
			...
			_load_attributes : function(xml_node) {
				...
				for (var i = 0; i < children.length; i++) {
					...
					if (attr.nodeType == 1 && attr.tagName === 'attribute') {
						...
						attr_data[attr.getAttribute('NAME')] = attr
								.getAttribute('VALUE');
					}
				}
				return attr_data;
			...
			_buildmap : function(node, xmllines) {
				...
				var pos = null;
				if (!!node.parent && node.parent.isroot) {
					pos = node.direction === jm.direction.left
							? 'left'
							: 'right';
				}
				xmllines.push('<node');
				...
				if (children.length > 0 || node_data != null) {
					...
					if (node_data != null) {
						for (var k in node_data) {
							xmllines.push('<attribute NAME=\"' + k
									+ '\" VALUE=\"' + node_data[k] + '\"/>');
						}
					}
					// for children
	...
	jm.util = {
		ajax : {
			...
			request : function(url, param, method, callback) {
				var a = jm.util.ajax, p = null, tmp_param = [], k;
				for (k in param) {
		...
		canvas : {
			easing_gauss : function(t, b, c, d) {
				var x = t * 4 / d;
				return (1 - Math.pow(Math.E, -(x * x) / 2)) * c + b;
			},
			lineto : function(ctx, p1, p2, opts) {
				var x1 = p1.x, y1 = p1.y, x2 = p2.x, y2 = p2.y, ztf = jm.util.canvas.easing_gauss;
				ctx.moveTo(x1, y1);
				ctx.beginPath();
				var l = x2 - x1;
				var c = y1 - y2;
		...
		file : {
			...
			save : function(file_data, type, name) {
				...
				if (typeof $w.Blob == "function") {
					...
					var BlobBuilder = $w.BlobBuilder || $w.MozBlobBuilder
							|| $w.WebKitBlobBuilder || $w.MSBlobBuilder;
				...
				for (var o in a) {
					if (o in b) {
						if (typeof b[o] === 'object'
								&& Object.prototype.toString.call(b[o])
		...
		uuid : {
			newid : function() {
				...
				var _uuid = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(
						/[xy]/g, function(c) {
	...
	jm.prototype = {
		init : function() {
			...
			var opts_view = {
				...
				extensionPanel : opts.extensionPanel,
				listeners : opts.listeners
			};
			// create instance of function provider
			this.data = new jm.data_provider(this, {
						loadingUrl : opts.loadingUrl
					});
			...
			if (opts.listeners && opts.listeners.afterInit) {
				opts.listeners.afterInit.call(this, this);
			}
		},
		...
		_event_bind : function() {
			this.view.event_bind(this, null, null, this.click_handle,
					this.dblclick_handle);
		},
		...
		click_handle : function(e) {
			...
			if (isnode) {
				...
				if (node && node._data.layout.isLoading) {
					return
				}
		...
		add_node : function(parent_node, nodeid, topic, data, leaf) {
			//if (this.get_editable()) {
		...
		insert_node_before : function(node_before, nodeid, topic, data, leaf) {
			if (this.get_editable()) {
				var node = this.mind.insert_node_before(node_before, nodeid,
						topic, data, leaf);
				if (!!node) {
					this.view.add_node(node);
					this.layout.layout();
		...
		insert_node_after : function(node_after, nodeid, topic, data, leaf) {
			if (this.get_editable()) {
				var node = this.mind.insert_node_after(node_after, nodeid,
						topic, data, leaf);
				if (!!node) {
					this.view.add_node(node);
					this.layout.layout();
		...
		select_node : function(node) {
			...
			if (opts.listeners && opts.listeners.selectNode) {
				opts.listeners.selectNode.apply(this, [this, node]);
			}
		},
		...
		get_extpanel : function() {
			return this.view.extension_panel
		}
	};

	// ============= data provider =============================================

	jm.data_provider = function(jm, opts) {
		this.jm = jm;
		this.opts = opts;
	};
	...
	jm.data_provider.prototype = {
		...
		loadChildren : function(node, callback) {
			var me = this, url = me.opts.loadingUrl, path = node.getPath();
			if (url && url.length) {
				var cb = function(datas) {
					if (datas && datas.length) {
						me.add_remote_node(node, datas);
						if (typeof(callback) == 'function') {
							callback(node, datas);
						}
					}
				};
				jm.util.ajax.request(url, {
							node : path
						}, 'POST', function(datas) {
							cb(datas);
						});
		...
		add_remote_node : function(node, datas) {
			for (var i = 0; i < datas.length; i++) {
				var n = datas[i];
				if (n.leaf == undefined) {
					n.leaf = !(n.children && n.children.length)
				}
				var c_node = this.jm.add_node(node, n.id, n.topic, n, n.leaf);
				if (n.children && n.children.length) {
					this.add_remote_node(c_node, n.children);
				}
			}
		},
	...
	jm.layout_provider.prototype = {
		...
		_layout_direction_root : function() {
			...
			if (this.isside) {
				...
				while (i--) {
					this._layout_direction_side(children[i],
							jm.direction.right, i);
				}
			} else {
				var i = children_count;
				var subnode = null;
				while (i--) {
					...
					if (subnode.direction == jm.direction.left) {
						this._layout_direction_side(subnode, jm.direction.left,
								i);
					} else {
						this._layout_direction_side(subnode,
								jm.direction.right, i);
					}
				}
				/*
				 * var boundary = Math.ceil(children_count/2); var i =
				 * children_count; while(i--){ if(i>=boundary){
				 * this._layout_direction_side(children[i],jm.direction.left,
				 * children_count-i-1); }else{
				 * this._layout_direction_side(children[i],jm.direction.right,
				 * i); } }
				 */
		...
		layout_offset : function() {
			...
			layout_data.outer_height_left = this
					._layout_offset_subnodes(left_nodes);
			layout_data.outer_height_right = this
					._layout_offset_subnodes(right_nodes);
			this.bounds.e = node._data.view.width / 2;
			this.bounds.w = 0 - this.bounds.e;
			// _console.debug(this.bounds.w);
			this.bounds.n = 0;
			this.bounds.s = Math.max(layout_data.outer_height_left,
		...
		_layout_offset_subnodes : function(nodes) {
			...
			while (i--) {
				...
				node_outer_height = Math.max(node._data.view.height,
						node_outer_height);

				layout_data.outer_height = node_outer_height;
				layout_data.offset_y = base_y - node_outer_height / 2;
				layout_data.offset_x = this.opts.hspace * layout_data.direction
						+ pd.view.width
						* (pd.layout.direction + layout_data.direction) / 2;
				if (!node.parent.isroot) {
					layout_data.offset_x += this.opts.pspace
							* layout_data.direction;
				}
		...
		_layout_offset_subnodes_height : function(nodes) {
			...
			while (i--) {
				...
				node_outer_height = this
						._layout_offset_subnodes_height(node.children);
				if (('isexpand' in layout_data) && !layout_data.isexpand) {
					node_outer_height = 0;
				}
				node_outer_height = Math.max(node._data.view.height,
		...
		get_node_point : function(node) {
			...
			var p = {};
			p.x = offset_p.x + view_data.width
					* (node._data.layout.direction - 1) / 2;
		...
		get_node_point_out : function(node) {
			...
			if (pout_cache.x == -1 || pout_cache.y == -1) {
				if (node.isroot) {
					...
					var view_data = node._data.view;
					var offset_p = this.get_node_offset(node);
					pout_cache.x = offset_p.x
							+ (view_data.width + this.opts.pspace)
							* node._data.layout.direction;
		...
		setLoading : function(node, flag) {
			if (node.isroot) {
				return;
			}
			var layout_data = node._data.layout, expander = node._data.view.expander;
			if (flag) {
				layout_data.isLoading = true;
				expander.innerHTML = '';
				expander.className = 'loading';
			} else {
				layout_data.isLoading = false;
				expander.className = '';
			}
		},
		...
		toggle_node : function(node) {
			...
			var layout_data = node._data.layout, me = this, isexpand = (node.children && node.children.length);
			if ('isexpand' in layout_data) {
				isexpand = layout_data.isexpand;
			}
			if (isexpand) {
				me.collapse_node(node);
			} else {
				if (node.children && node.children.length) {
					me.expand_node(node);
				} else {
					me.setLoading(node, true);
					me.jm.data.loadChildren(node, function() {
								me.expand_node(node);
								me.setLoading(node, false);
							});
				}
			}
		},
		...
		part_layout : function(node) {
			...
			if (!!root) {
				...
				if (node._data.layout.direction == jm.direction.right) {
					root_layout_data.outer_height_right = this
							._layout_offset_subnodes_height(root_layout_data.right_nodes);
				} else {
					root_layout_data.outer_height_left = this
							._layout_offset_subnodes_height(root_layout_data.left_nodes);
				}
				this.bounds.s = Math.max(root_layout_data.outer_height_left,
	...
	jm.view_provider.prototype = {
		init : function() {
			...
			me.container = $g(me.opts.container);
			if (!me.container) {
				_console
						.error('the options.view.container was not be found in dom');
				return;
			}
			me.e_panel = $c('div');
			me.e_canvas = $c('canvas');
			me.e_nodes = $c('jmnodes');
			me.e_editor = $c('input');

			me.e_panel.className = 'jsmind-inner';
			me.e_panel.appendChild(this.e_canvas);
			me.e_panel.appendChild(this.e_nodes);

			// theme chooser
			var e_theme_chooser = $c('div'), icon = $c('div'), themes_cnt = $c('div'), t, title;
			icon.className = 'icon';
			themes_cnt.className = 'cnt';
			e_theme_chooser.className = 'theme-chooser';
			for (var i in THEME_OPTIONS) {
				if (THEME_OPTIONS.hasOwnProperty(i)) {
					t = $c('div');
					title = $c('span');
					title.innerHTML = THEME_OPTIONS[i].title;
					t.appendChild(title);
					t.className = 'theme';
					t.name = i;
					t.style.background = THEME_OPTIONS[i].symbleColor;
					themes_cnt.appendChild(t);
				}
			}

			e_theme_chooser.appendChild(icon);
			e_theme_chooser.appendChild(themes_cnt);
			me.e_panel.appendChild(e_theme_chooser);

			me.e_editor.className = 'jsmind-editor';
			me.e_editor.type = 'text';
			...
			jm.util.dom.add_event(themes_cnt, 'click', function(e) {
						var theme;
						if (e.target.className == 'theme') {
							theme = e.target;
						}
						if (!theme && e.target.parentNode.className == 'theme') {
							theme = e.target.parentNode;
						}
						if (theme) {
							me.jm.options.theme = theme.name;
							me.reset_theme();
						}
					});

			me.container.appendChild(me.e_panel);

			me.init_canvas();

			if (me.opts.extensionPanel) {
				var ext_opts = {};
				jm.util.json.merge(ext_opts, me.opts.extensionPanel);
				jm.util.json.merge(ext_opts, {
							showExtensionPanel : me.opts.listeners
									? me.opts.listeners.showExtensionPanel
									: null,
							hideExtensionPanel : me.opts.listeners
									? me.opts.listeners.hideExtensionPanel
									: null
						});
				me.extension_panel = new jm.view_provider.extension_panel(
						me.jm, ext_opts, me.e_panel);
				me.extension_panel.init();
			}
		...
		event_bind : function(obj, fn_mouseover, fn_mouseout, fn_click,
				fn_dblclick) {
			if (!!fn_mouseover) {
				jm.util.dom.add_event(this.e_nodes, 'mouseover', function(e) {
							fn_mouseover.call(obj, e);
						});
			}
			if (!!fn_mouseout) {
				jm.util.dom.add_event(this.e_nodes, 'mouseout', function(e) {
							fn_mouseout.call(obj, e);
						});
			}
			if (!!fn_click) {
			...
			if (!!fn_dblclick) {
				jm.util.dom.add_event(this.e_nodes, 'dblclick', function(e) {
							fn_dblclick.call(obj, e);
						});
			}
		},
		get_nodeid : function(element) {
			...
			if (element.id == this.opts.container) {
				return '';
			}
			if (element.parentNode != null) {
				return this.get_nodeid(element.parentNode);
			}
		},

		is_real_node : function(element) {
		...
		is_node : function(element) {
			...
			var flag = this.is_real_node(element);
			if (!flag && element.parentNode != null) {
				return this.is_node(element.parentNode);
			}
			return flag;
		},
		...
		reset_theme : function() {
			...
			if (!!theme_name) {
				this.e_panel.className = 'jsmind-inner jmnodes-' + theme_name
						+ '-bg';
			...
			if (this.opts.autoExpand) {
				...
				try {
					if (this.opts.container) {
						var c_min_h = min_height + 10;
						document.getElementById(this.opts.container).style.height = (c_min_h + 20)
								+ "px";
						if (client_h > c_min_h) {
							client_h = c_min_h;
						}
		...
		create_node_element : function(node) {
			...
			if (node.isroot) {
				...
				d.className = 'level-' + (node.level > 2 ? 2 : node.level);
		...
		select_node : function(node) {
			if (!!this.selected_node) {
				this.selected_node._data.view.element.className = this.selected_node._data.view.element.className
		...
		show_nodes : function() {
			...
			for (var nodeid in nodes) {
				...
				node_element.style.left = (_offset.x + p.x + (node.level == 1
						? node.direction * -2
						: 0))
						+ 'px';
				node_element.style.top = (_offset.y + p.y + (node.level >= 2
						? -15
						: 0))
						+ 'px';
		...
		clear_lines : function() {
			jm.util.canvas.clear(this.canvas_ctx, 0, 0, this.size.w,
					this.size.h);
		},
		...
		show_lines : function() {
			...
			if (this.jm.mind && this.jm.mind.nodes) {
				...
				for (var nodeid in nodes) {
					...
					if (('visible' in node._data.layout)
							&& !node._data.layout.visible) {
						continue;
					}
					pin = this.layout.get_node_point_in(node);
					pout = this.layout.get_node_point_out(node.parent);
					this.draw_line(node, pout, pin, _offset);
				}
			}
		},

		draw_line : function(node, pin, pout, offset) {
			var theme = this.jm.options.theme, start, end, end2;
			start = {
				x : pin.x + offset.x,
				y : pin.y + offset.y
			};
			...
			jm.util.canvas.lineto(this.canvas_ctx, start, end,
					THEME_OPTIONS[theme].canvasOpts);
			if (node.level >= 2) {
				end2 = {
					x : end.x + node.direction * node._data.view.width,
					y : end.y
				}
				jm.util.canvas.lineto(this.canvas_ctx, end, end2,
						THEME_OPTIONS[theme].canvasOpts);
			}
		}
	};
	...
	// extension panel
	jm.view_provider.extension_panel = function(jm, opts, cnt) {
		var me = this;
		me.jm = jm;
		me.opts = opts;
		me.cnt = cnt;
	...
	jm.view_provider.extension_panel.prototype = {
		constructor : jm.view_provider.extension_panel,
		init : function() {
			var me = this, opts = me.opts, cnt = me.cnt;
			me.ext_panel = $c('div');
			me.ext_panel.className = 'extension-panel';
			me.ext_panel.style.float = opts.align;
			me.ext_panel.style.width = opts.width;

			cnt.appendChild(me.ext_panel);
		},
		show : function() {
			var me = this;
			me.ext_panel.style.display = 'block';
			if (me.opts.showExtensionPanel) {
				me.opts.showExtensionPanel.apply(me.jm, [me.jm, me]);
			}
		},
		hide : function() {
			var me = this;
			me.ext_panel.style.display = 'none';
			if (me.opts.hideExtensionPanel) {
				me.opts.hideExtensionPanel.apply(me.jm, [me.jm, me]);
			}
		},
		getDom : function() {
			return this.ext_panel
		}
	};
	...
	jm.shortcut_provider.prototype = {
		...
		handle_addbrother : function(_jm, e) {
			...
			if (!!selected_node && !selected_node.isroot) {
				var nodeid = jm.util.uuid.newid();
				var node = _jm.insert_node_after(selected_node, nodeid,
						'New Node');
				if (!!node) {
					_jm.select_node(nodeid);
					_jm.begin_edit(nodeid);
