(function($) {
	$.fn.slideBox = function(opts) {
		...
		function scroll(index, callback) {
			...
			c.control
					&& $(c.control).find(".btn").eq(cursor).addClass("active");
			$(self).stop().animate({
						left : left,
						marginLeft : 0
					}, c.speed, function() {
		...
		$(this).bind("mousedown", function(e) {
			// 不响应事件
			if (typeof c.cancel == "string" ? $(e.target).parents()
					.add(e.target).filter(c.cancel).length : false) {
				return;
			}
			var s = e.clientX;
		...
		function slide() {
			...
			var obstacle;
			if (typeof c.obstacle === "string"
					&& /^[0-9]{1,2}%$/.test(c.obstacle)) {
