/*
 * JQuery zTree core 3.4 http://zTree.me/
 * 
 ...
 * 
 * email: hunter.z@263.net Date: 2012-09-03
 */
(function($) {
		...
		getNodesByParamFuzzy : function(setting, nodes, key, value) {
			...
			for (var i = 0, l = nodes.length; i < l; i++) {
				if (typeof nodes[i][key] == "string"
						&& nodes[i][key].toLowerCase().indexOf(value
								.toLowerCase()) > -1) {
					result.push(nodes[i]);
				}
	...
	view = {
		...
		makeDOMNodeNameBefore : function(html, setting, node) {
			...
			if (node.isParent && setting.view.strongFloder) {
		...
		makeNodeIcoClass : function(setting, node) {
			...
			if (!node.isAjaxing) {
				...
				if (node.isParent) {
					icoCss.push(node.open
							? consts.folder.OPEN
							: consts.folder.CLOSE);
					if (setting.view.strongFloder) {
						strongCss = consts.folder.STRONG;
					}
				} else {
			...
			return strongCss + " button " + icoCss.join('_');
		},
