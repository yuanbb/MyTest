...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addJSONData = function(data, rcnt, more, adjust) {
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
					...
					if (ts.firstElementChild) {
						$(ts.firstElementChild).append(rowData.join(''));//eastcom edit 修改兼容ie浏览器，加载抱错的问题
						//ts.firstElementChild.innerHTML += rowData.join(''); // append to innerHTML of tbody which contains the first row (.jqgfirstrow)
					} else {
						$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first").append(rowData.join(''));
					}
		...
		//eastcom add 针对新创建的表格，增加onWindowResize方法{id:ts.id}
		if(ts.p.windowresize){$(ts).jqGrid("onWindowResize");$(ts).jqGrid("jqGridResize",{compel:true});}
	});
};
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			//eastcom add 兼容处理如果新的宽度大于当前表格所有列的宽度，使用自动列宽，使列宽变大，充满新的宽度
			if (shrink === false) {
				var _tblW = $t.p.tblwidth;
				if (_tblW<(nwidth-1)) {
					shrink = true;
				}
			}
			...
			if(shrink===true) {
				...
				aw = nwidth-brd*vc-gw;
				if(!isNaN($t.p.height)) {
					//$t.rows.length === 1 to $t.rows.length >= 1 eastcom edit 修改问题，解决浏览器分辨率变化后计算的宽度有问题
